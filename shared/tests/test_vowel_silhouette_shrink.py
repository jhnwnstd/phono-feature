...
# Patch target: the module that OWNS the shrink tunables (``shrink``).
# ``vowel_silhouette`` lives one layer down in ``silhouette``. Patching
# either compat facade would rebind the facade's alias while the solver
...
def test_stage2_disabled_by_default_for_silhouette_consistency() -> None:
    ...
    constant in chart_style/shrink, update this test to
    document the new value + rationale, and visual-verify that
    the per-inventory slant variation is desired.
    """
    assert shrink_mod._VOWEL_SLANT_CHANGE_CAP_FRAC == 0.0, (
...
def test_stage2_disabled_returns_stage1() -> None:
    ...
    """
    with patched_module_attr(shrink_mod, "_VOWEL_SLANT_CHANGE_CAP_FRAC", 0.0):
...
def test_hayes_silhouette_within_slant_cap(
    ...
    max_allowed_delta = (
        shrink_mod._VOWEL_SLANT_CHANGE_CAP_FRAC * canonical_slant
    )
