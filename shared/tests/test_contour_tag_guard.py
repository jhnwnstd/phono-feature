...
from gen_contour_tag_reach import _reach  # noqa: E402

_FIXTURE = json.loads((_FIXTURES / "contour_tag_reach.json").read_text())

# Modules that compute set / class membership from feature or class
# queries. The display tag must never appear in their source, or a class
# query could be accidentally sourced from a presentation label.
_MEMBERSHIP_MODULES = [
    "phonology_shared/data/tiers.py",
...
@pytest.mark.parametrize("rel", _MEMBERSHIP_MODULES)
def test_membership_modules_never_reference_the_display_tag(rel: str) -> None:
    """No membership/query module names the Contour Consonants tag, by
    its constant OR its literal value. Freezes the separation: a class
    query cannot be sourced from the display artifact."""
    src = (_SHARED_SRC / rel).read_text()
    assert "CONTOUR_GROUP_NAME" not in src, rel
    assert CONTOUR_GROUP_NAME not in src, rel
...
def test_fixture_is_internally_consistent() -> None:
    """Schema + totals match the recorded segment map."""
...
def test_fixture_matches_live_engine_on_a_sample() -> None:
    """Over a fast sample of inventories, every tagged segment (a) is in
    the fixture, (b) reaches exactly its recorded classes when recomputed
    from the live tiers, and (c) is returned by the engine's ∃ query for a
    feature it specifies. Catches drift in the gate, the tag rule, or the
    reach computation without the full-corpus loop."""
    provider, inventories = _provider()
    recorded = _FIXTURE["segments"]
    checked = 0
    for entry in inventories[:80]:
        inv = materialize_phoible_inventory(provider, entry["id"])
        eng = FeatureEngine(inv)
        for seg in eng.grouped_segments.get(CONTOUR_GROUP_NAME, []):
            assert seg in recorded, f"tagged {seg!r} missing from fixture"
            tiers = {
                k.lower(): tuple(v) for k, v in inv.sequences(seg).items()
            }
            assert _reach(tiers) == recorded[seg], seg
            checked += 1
    assert checked >= 20, f"sample too small ({checked}); fixture unverified"
