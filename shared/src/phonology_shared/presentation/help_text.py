...
VOWELS_HELP_HTML: str = (
    ...
    " region. If the inventory uses tense or ATR contrasts, these rows are"
    " refined. A [-tense] high vowel maps to Near-close. A [+tense] mid vowel"
    " maps to Close-mid, and a [-tense] mid vowel maps to Open-mid. A mid"
    " vowel with no tense or ATR value maps to the plain Mid row.</p>"
    "<p><b>Low vowels and /a/</b><br>Low vowels normally remain on the Open"
    " row. This includes central /a/, which is represented as [-front,"
    ...
    " keep all low vowels on Open, but the split is enabled here by"
    " default.</p>"
    "<p>A lone low vowel is handled specially. If the inventory has exactly"
    " one low vowel and no front vowels, that vowel is drawn in the"
    " bottom-left corner instead of the center of the bottom edge.</p>"
