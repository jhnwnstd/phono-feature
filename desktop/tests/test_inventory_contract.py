...
def test_parse_rejects_too_many_features() -> None:
    ...
    Largest bundled inventory uses 30 features; cap at 40 leaves
    room for slightly-larger customs."""
    with pytest.raises(ValidationError) as ex:
        Inventory.parse(
            {"features": [f"F{i}" for i in range(500)], "segments": {}}
        )
    assert "40" in " ".join(ex.value.issues)


def test_parse_rejects_too_many_segments() -> None:
    """Hard cap at 200 segments; largest bundled (Hayes) is 140."""
    with pytest.raises(ValidationError) as ex:
        Inventory.parse(
            {
                "features": ["V"],
    ...
    assert "200" in " ".join(ex.value.issues)
...
def test_inventories_dir_resolves_to_bundled_dir() -> None:
    ...
    assert inv_dir.parent.name == "desktop"
