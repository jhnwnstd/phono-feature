...
from phonology_features.gui.shared.view_models import (
    ANALYSIS_TABS_REQUIRED_KEYS,
    ANALYSIS_TABS_VALID_CLASS_STATES,
...
# ---------------------------------------------------------------------------
# Stage 3: AnalysisTabsPayload shape contract.
#
# Pins the bridge payload schema both UIs read. A drop or rename
# on the producer side (``_seg_tabs`` / ``_feat_tabs``) without a
# parallel update on the consumer side (desktop ``set_sections``
# and web ``setAnalysisTabs``) was silent until Stage 3; now it
# fails CI via these tests AND raises at the bridge boundary in
# ``analyze_segments`` / ``analyze_features`` via
# ``validate_analysis_tabs_payload``.
# ---------------------------------------------------------------------------


def test_analysis_tabs_payload_keys_complete_for_seg_mode() -> None:
    """Every required key must appear in the SEG-mode payload for
    both empty and populated selections."""
    engine = _engine("hayes_features.json")
    for segs in ([], ["b"], ["b", "d", "ɡ"]):
        summary = summarize_segment_selection(engine, list(segs))
        tabs = summary["analysis_tabs"]
        assert set(tabs.keys()) == set(ANALYSIS_TABS_REQUIRED_KEYS), (
            f"SEG analysis_tabs for {segs} has keys"
            f" {sorted(tabs.keys())}; expected"
            f" {sorted(ANALYSIS_TABS_REQUIRED_KEYS)}"
        )


def test_analysis_tabs_payload_keys_complete_for_feat_mode() -> None:
    """Every required key must appear in the FEAT-mode payload for
    both empty and populated queries."""
    engine = _engine("hayes_features.json")
    for spec in ({}, {"Voice": "+"}, {"Voice": "+", "Nasal": "-"}):
        summary = summarize_feature_query(engine, dict(spec))
        tabs = summary["analysis_tabs"]
        assert set(tabs.keys()) == set(ANALYSIS_TABS_REQUIRED_KEYS), (
            f"FEAT analysis_tabs for {spec} has keys"
            f" {sorted(tabs.keys())}; expected"
            f" {sorted(ANALYSIS_TABS_REQUIRED_KEYS)}"
        )


def test_analysis_tabs_contrasts_enabled_is_mode_driven() -> None:
    """The Contrasts tab is enabled in SEG and disabled in FEAT,
    irrespective of selection contents. This is the contract the
    plan stage 3 promoted from a comment to a runtime assertion."""
    engine = _engine("hayes_features.json")
    for segs in ([], ["b"], ["b", "d", "ɡ"]):
        tabs = summarize_segment_selection(engine, list(segs))["analysis_tabs"]
        assert tabs["contrasts_enabled"] is True, (
            f"SEG mode must always enable Contrasts; got False for"
            f" segs={segs}"
        )
    for spec in ({}, {"Voice": "+"}, {"Voice": "+", "Nasal": "-"}):
        tabs = summarize_feature_query(engine, dict(spec))["analysis_tabs"]
        assert tabs["contrasts_enabled"] is False, (
            f"FEAT mode must never enable Contrasts; got True for"
            f" spec={spec}"
        )


def test_analysis_tabs_class_state_in_valid_set() -> None:
    """Class-tab colour cue is one of the three documented strings;
    no other value should sneak through a future refactor."""
    engine = _engine("hayes_features.json")
    seen_seg = set()
    for segs in ([], ["b"], ["b", "d"], ["b", "d", "ɡ"]):
        tabs = summarize_segment_selection(engine, list(segs))["analysis_tabs"]
        seen_seg.add(tabs["class_state"])
    seen_feat = set()
    for spec in ({}, {"Voice": "+"}):
        tabs = summarize_feature_query(engine, dict(spec))["analysis_tabs"]
        seen_feat.add(tabs["class_state"])
    assert seen_seg <= set(ANALYSIS_TABS_VALID_CLASS_STATES)
    assert seen_feat <= set(ANALYSIS_TABS_VALID_CLASS_STATES)


def test_validate_analysis_tabs_payload_accepts_canonical_output() -> None:
    """The bridge validator must accept everything ``_seg_tabs`` /
    ``_feat_tabs`` emit. A round-trip via the validator is the
    closest local mirror of what the bridge does on every
    ``analyze_segments`` / ``analyze_features`` call."""
    engine = _engine("hayes_features.json")
    for payload in (
        summarize_segment_selection(engine, [])["analysis_tabs"],
        summarize_segment_selection(engine, ["b"])["analysis_tabs"],
        summarize_segment_selection(engine, ["b", "d"])["analysis_tabs"],
        summarize_feature_query(engine, {})["analysis_tabs"],
        summarize_feature_query(engine, {"Voice": "+"})["analysis_tabs"],
    ):
        validated = validate_analysis_tabs_payload(payload)
        assert validated is payload  # identity, not copy


def test_validate_analysis_tabs_payload_rejects_missing_keys() -> None:
    """The validator catches the canonical drift: a key was dropped
    by an upstream refactor."""
    ...
    del payload["contrasts"]
    with _pytest.raises(ValueError, match="missing required keys"):
        validate_analysis_tabs_payload(payload)


def test_validate_analysis_tabs_payload_rejects_extra_keys() -> None:
    """An unexpected key fails too; new fields must extend the
    required-keys set so the contract stays explicit."""
    import pytest as _pytest

    engine = _engine("hayes_features.json")
    payload = dict(summarize_segment_selection(engine, ["b"])["analysis_tabs"])
    payload["extra_key"] = "surprise"
    with _pytest.raises(ValueError, match="unexpected keys"):
        validate_analysis_tabs_payload(payload)


def test_validate_analysis_tabs_payload_rejects_bad_class_state() -> None:
    """Invalid class_state values are caught."""
    import pytest as _pytest

    engine = _engine("hayes_features.json")
    payload = dict(summarize_segment_selection(engine, ["b"])["analysis_tabs"])
    payload["class_state"] = "rainbow"
    with _pytest.raises(ValueError, match="class_state"):
        validate_analysis_tabs_payload(payload)


def test_validate_analysis_tabs_payload_rejects_non_dict() -> None:
    """Boundary check: list / None / int are not valid payloads."""
    import pytest as _pytest

    for bad in (None, [], "string", 42):
        with _pytest.raises(ValueError, match="must be a dict"):
            validate_analysis_tabs_payload(bad)
